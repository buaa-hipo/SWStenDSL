...
void laplaceAddInput(double value_arg0[72][18][16], double value_arg1[72][18][16]);
void laplaceAddInput_iteration(double value_arg0[72][18][16], double value_arg1[72][18][16]);
    ...
    for (i = 0; i < 72; i++) {
        for (j = 0; j < 18; j++) {
            for (k = 0; k < 16; k++) {
                ...
                // input[i][j][k] = i*j*k;
                double rand_num = rand() % 50;
                input_mpe[i][j][k] = rand_num;
                input_spe[i][j][k] = rand_num;
                // 毒化out_mpe和output_spe内存
                output_mpe[i][j][k] = rand_num;
                output_spe[i][j][k] = rand_num;
            }
        }
    }

    // laplaceAddInput(input, output_spe);
    laplaceAddInput_iteration(input_spe, output_spe);
            ...
            for (j = 1; j < 17; j++) {
                for (k = 0; k < 16; k++) {
                    output_mpe[i][j][k] = 
                        ...
                        4.0 * input_mpe[i][j][k] + input_mpe[i][j][k];
                }
            }
        }
        for (i = 1; i < 71; i++) {
            for (j = 1; j < 17; j++) {
                for (k = 0; k < 16; k++) {
                    input_mpe[i][j][k] = 
                        ...
                        4.0 * output_mpe[i][j][k] + output_mpe[i][j][k];
                }
            }
        }
    }
    ...
    for (i = 1; i < 71; i++) {
        for (j = 1; j < 17; j++) {
            for (k = 0; k < 16; k++) {
                ...
                double value = input_mpe[i][j][k] - input_spe[i][j][k];
